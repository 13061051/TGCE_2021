...
def evaluate_test_acc(test_x, test_y,test_x_base, model):
    #
    ...
    best_p = 0
    best_R = 0
    ths = [0.02 * i for i in range(20,50)]
    golden_test = load_golden_set('data/original/golden_valid2.csv')
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(batch_train_num):
            ...
            x_base = torch.Tensor(train_x_other[i * batch:i * batch + batch]).to(device)
            out = user_macth_model(input_x,x_base)

            loss_out = criterion(out, input_y)
            loss = loss_out
            loss.backward()
            optimizer.step()
        ...
        F1, th = evaluate_validation(valid_x=x_valid, valid_y=valid_y,
                                     model=user_macth_model,valid_x_base=valid_x_other)
        print('')
        if F1>best_F1:
            best_F1=F1
