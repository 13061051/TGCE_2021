...
def read_data_sample(url_index_l1, url_index_l2, url_index_l3, url_index_l4,deep_path=10):
	...
	for i in range(len(test_pairs)):
		...
		if (pair[0], pair[1]) in golden_test or (pair[1], pair[0]) in golden_test:
			...
			test_y.append(0)
	return x_train, train_y, x_test, test_y, train_x_other, test_x_other
...
if __name__ == '__main__':
	...
	x_train_src, train_y_src, x_test_src, test_y_src, train_x_other_src, test_x_other_src = read_data_sample(
	...
	for epoch in range(args.epochs):
		...
		# 每个回合随机抽取5%数据
		random_index=[i for i in range(len(x_train_src))]
		...
		for k in range(len(random_index)):
			x_train_new.append(x_train_src[random_index[k]])
			y_train_new.append(train_y_src[random_index[k]])
			x_other_new.append(train_x_other_src[random_index[k]])
		# validation set should be choosen by training dataset
		valid_size = int(0.05 * len(x_train_src))
		valid_y = y_train_new[0:valid_size]
		valid_x_other = x_other_new[:valid_size]
		x_valid = x_train_new[0:valid_size]
		# for training
		train_y = y_train_new[valid_size:]
		train_x_other = x_other_new[valid_size:]
		x_train = x_train_new[valid_size:]

		train_y = np.array(train_y)
		valid_y = np.array(valid_y)
		test_y = np.array(test_y_src)
	...
	user_macth_model = user_macth_model.to(device)
	user_macth_model.eval()
	evaluate_test_acc(test_x=x_test_src, test_y=test_y_src, model=user_macth_model)
