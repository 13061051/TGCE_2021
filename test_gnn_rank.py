...
def read_data_sample(url_index_l1, url_index_l2, url_index_l3, url_index_l4, deep_path=10):
    ...
    with open("data/original/uids_random_edges/uid_random_edges_n2_d" + str(deep_path) + ".txt", 'r',
    ...
    X_test=[]
    fr=open('data/original/golden_valid2_ranking.csv','r',encoding='utf-8')
    lines=fr.readlines()
    for line in lines:
        item=[]
        uids_=line.replace('\n','r').split(',')
        ...
        pair_0=uids_[0]
        for j in range(1,30):
            item.append([uid2seq[pair_0],uid2seq[uids_[j]]])
        X_test.append(item)
    print(len(X_test),' link pair num')

    X_train=[]
    fr = open('data/original/golden_train_ranking.csv', 'r', encoding='utf-8')
    lines = fr.readlines()
    for line in lines:
        item = []
        uids_ = line.replace('\n', 'r').split(',')
    ...
    print(len(X_test),len(X_train), ' link pair num')
    return X_test
...
def evaluate_validation(valid_x, valid_y, model):
    ...
    for th in ths:
        ...
        if F1 > best_F1:
            ...
            best_F1 = F1
            best_R = R
    print(time_now_str() + " Valid Pre: " + str(best_p) + " Rec: " + str(best_R) + " F1: " + str(
        best_F1) + " best Th: " + str(best_th))
...
def evaluate_test_acc(test_x, model,model_path=''):
    ...
    MRR10=0
    batch_size=10
    batch_num = int(len(test_x)/batch_size+1)
    print('test batch num',batch_num)
    predicts=[]

    for i in range(batch_num):
        input_x_original = test_x[i*batch_size:i*batch_size+batch_size]
        num=len(input_x_original)
        if num==0:
            break
        input_x=[]
        for j in range(num):
            temp=input_x_original[j]
            for item in temp:
                input_x.append(item)
        # input_x_other = trans_to_cuda(torch.Tensor(input_x_other))
        out = model(input_x)
        out_array = out.detach().cpu().numpy()
        out_array=np.reshape(out_array,[num,-1])
        #例如np.argsort()[-1]即输出x中最大值对应的index，np.argsort()[-2]即输出x中第二大值对应的index，依此类推。。
        for t in range(num):
            out_item=out_array[t]
            out_index=np.argsort(-1*out_item)#乘上-1做个变化
            #
            index=0
            for j in range(len(out_index)):
                if out_index[j] == 0:
                    index = j
            if 0 in out_index[0:3]:
                HR3=HR3+1
            if 0 in out_index[0:5]:
                HR5=HR5+1
            if 0 in out_index[0:10]:
                HR10=HR10+1
                MRR10=MRR10+(1/(1+index))
            predicts.append(index)
        if i%100==0 and i>0:
            print(time_now_str()+"HR10,HR5,HR3,i*batch_size+num")
            print(HR10,HR5,HR3,i*batch_size+num)
            print(predicts[-10:])
            print('------')
    print(len(predicts)," len predicts")
    try:
        fo = open('save_results/' + model_path + "_predict_ranks.txt", 'w', encoding='utf-8')
        for i in range(len(predicts)):
            fo.write(str(predicts[i])+"\n")
        fo.close()
    except:
        print('error file')
    HR3=HR3/len(test_x)
    HR5 = HR5 /len(test_x)
    HR10 = HR10 / len(test_x)
    MRR10=MRR10/len(test_x)
    print(time_now_str() + ': ranking result', 'HR3, HR5, HR10, MRR10')
    print(time_now_str()+': ranking result',HR3,HR5,HR10,MRR10)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--model_path', type=str, default='GRU_POSGAT_d16', help='the model_path'
    )
    ...
    user_macth_model = gnn_models.GNN_Match(num_items=num_items, embeds=embeds, embedding_dim=embedding_dim,
                                            drop_out=args.dropout, gnn_function=args.gnn_function)
    ##
    ...
    evaluate_test_acc(test_x=X_test,model=user_macth_model,model_path=args.model_path)
